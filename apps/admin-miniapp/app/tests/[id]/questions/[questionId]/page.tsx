...
import { api, fetchTopics, fetchTest, updateQuestion } from '@/lib/api'
import { BottomNav } from '@/components/BottomNav'
import { ChevronLeft, Save, Plus, Trash2, CheckCircle, Circle, UploadCloud, Loader2 } from 'lucide-react'
import { cn } from '@/lib/utils'

export default function EditQuestionPage() {
    const { id: testId, questionId } = useParams()
    ...
    // Wait until test data is loaded to get the question
    ...
    useEffect(() => {
        if (test && test.questions && !isDataLoaded) {
            const question = test.questions.find((q: any) => q.id === questionId)
            if (question) {
                setContent(question.content)
                setTopicId(question.topic_id)
                setImageUrl(question.image_url || '')
                setOptions(question.options && question.options.length > 0 ? question.options.map((o: any) => ({
                    id: o.id,
                    content: o.content,
    ...
    }, [test, questionId, isDataLoaded, router])

    const editMutation = useMutation({
        mutationFn: async (data: any) => {
            return updateQuestion(testId as string, questionId as string, data)
        },
        onSuccess: () => {
    ...
    if (isTestLoading || !isDataLoaded) {
        return <div className="flex h-screen items-center justify-center bg-[#0f0f1a] text-white"><Loader2 className="animate-spin mr-2" /> Загрузка...</div>
    }
        ...
        <div className="flex flex-col pb-32 bg-[#0f0f1a] min-h-screen text-gray-100 font-sans">
            <header className="sticky top-0 bg-[#0f0f1a]/90 backdrop-blur-md border-b border-white/5 px-6 py-4 z-40 flex items-center justify-between">
                ...
                <div className="text-right">
                    <h1 className="text-base font-medium">Редактирование</h1>
                    <p className="text-[11px] text-gray-500 uppercase tracking-widest">Вопрос</p>
                </div>
            </header>
            ...
            <main className="p-6 flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <button 
                    disabled={!content || !topicId || options.some(opt => !opt.content) || editMutation.isPending}
                    onClick={() => editMutation.mutate({ content, topic_id: topicId, image_url: imageUrl, options: options.map(o => ({ content: o.content, is_correct: o.is_correct })) })}
                    className={cn(
                        ...
                        !content || !topicId || options.some(opt => !opt.content) || editMutation.isPending 
                            ? "bg-white/5 text-gray-500 cursor-not-allowed" 
                            : "bg-white text-black hover:bg-gray-100 active:scale-[0.98]"
                    )}
                >
                    {editMutation.isPending ? <Loader2 size={18} className="animate-spin"/> : <Save size={18} />}
                    {editMutation.isPending ? 'Сохранение...' : 'Обновить вопрос'}
                </button>
            </main>
        </div>
    )
